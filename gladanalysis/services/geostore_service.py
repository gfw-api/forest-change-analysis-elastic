...
class GeostoreService(object):
    ...
    def make_use_request(use_type, use_id):
        ...
        area_url = 'http://production-api.globalforestwatch.org/geostore/use/%s/%s' %(use_type, use_id)
    ...
    def make_gadm_request(iso_code, admin_id=None, dist_id=None):
        ...
        if not admin_id and not dist_id:
            geostore_url = 'https://production-api.globalforestwatch.org/geostore/admin/%s'%(iso_code)

        elif admin_id and not dist_id:
            geostore_url = 'https://production-api.globalforestwatch.org/geostore/admin/%s/%s'%(iso_code, admin_id)

        elif admin_id and dist_id:
            geostore_url = 'https://production-api.globalforestwatch.org/geostore/admin/%s/%s/%s'%(iso_code, admin_id, dist_id)
    ...
    def make_wdpa_request(wdpa_id):
        ...
        area_url = 'http://production-api.globalforestwatch.org/geostore/wdpa/%s' %(wdpa_id)
        r = requests.get(url=area_url)
        geostore_data = r.json()
        geostore = geostore_data['data']['id']
        area = geostore_data['data']['attributes']['areaHa']
